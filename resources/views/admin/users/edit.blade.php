...
<div class="card shadow-sm border-0 max-w-3xl mx-auto">
    <div class="card-body p-4">
        <h1 class="h4 mb-4 fw-bold">Editar Usuario: {{ $user->name }}</h1>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" novalidate>
            ...
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    <x-input name="name" label="Nombre completo" :required="true" placeholder="Juan Pérez" value="{{ old('name', $user->name) }}" />
                </div>
                <div class="col-12 col-md-6">
                    <x-input name="email" type="email" label="Correo electrónico" :required="true" placeholder="juan@empresa.com" value="{{ old('email', $user->email) }}" />
                </div>
                <div class="col-12 col-md-6">
                    <x-input name="phone" label="Teléfono" placeholder="+52 55 0000 0000" value="{{ old('phone', $user->phone) }}" />
                </div>
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        ...
                        <select name="role" id="user-role-select" class="form-select @error('role') is-invalid @enderror" required>
                            <option value="client" {{ old('role', $user->role) === 'client' ? 'selected' : '' }}>Cliente (Acceso a empresas específicas)</option>
                ...
                <div class="col-12 col-md-6">
                    <x-input name="password" type="password" label="Nueva contraseña" placeholder="Dejar en blanco para conservar" />
                </div>
                <div class="col-12 col-md-6">
                    <x-input name="password_confirmation" type="password" label="Confirmar nueva contraseña" placeholder="Repite la contraseña" />
                </div>
                                ...
                                <div class="overflow-y-auto" style="max-height: 200px;" id="companies-checkbox-list">
                                    @foreach ($companies as $company)
                                        <div class="form-check py-1 company-checkbox-item" data-company-name="{{ strtolower($company->name) }}">
                                            <input class="form-check-input" type="checkbox" name="company_ids[]" value="{{ $company->id }}" id="company-{{ $company->id }}" {{ in_array($company->id, old('company_ids', $userCompanyIds)) ? 'checked' : '' }}>
