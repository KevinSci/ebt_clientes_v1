...
<x-card class="ebt-project-summary mb-4">
...
{{-- ── Filters (GET-based) ──────────────────────────────────────────────── --}}
<div class="ebt-filters-panel mb-4">
...
@if ($search || $dateFrom || $dateTo)
    <div class="d-flex flex-wrap gap-2 mb-3" aria-label="Filtros activos">
        @if ($search)
            <span class="badge ebt-filter-tag">
...
@else
    <div class="ebt-feed" id="posts-feed" aria-label="Feed de publicaciones">
        ...
        @foreach ($posts as $post)
            <article class="ebt-feed__post" data-post-id="{{ $post->id }}">

                {{-- Post header --}}
                <div class="ebt-feed__post-header">
                    <div class="ebt-feed__post-icon" aria-hidden="true">
                        <i class="bi bi-megaphone-fill"></i>
                    </div>
                    <div>
                        <h2 class="h6 mb-0 fw-bold ebt-feed__post-title">{{ $post->title }}</h2>
                        @if ($post->published_at)
                            <time datetime="{{ $post->published_at->toIso8601String() }}"
                                  class="small text-muted ebt-feed__post-date">
                ...
                <div class="ebt-feed__post-body">
                    ...
                    <div class="ebt-read-more" data-needs-trim="{{ $needsTrim ? 'true' : 'false' }}">
                        <p class="ebt-read-more__text mb-0" style="white-space: pre-line">
                        ...
                        @if ($needsTrim)
                            <button type="button"
                                    class="btn btn-link ebt-read-more__btn p-0 mt-1 small"
                                    aria-expanded="false">
                                Ver más
                            </button>
                        @endif
                ...
                @if ($post->attachments->count() > 0)
                    <div class="ebt-feed__post-attachments mt-3">
                        <x-attachment-grid
